...
export async function getArticlesByJournalist(journalistId) {
  const response = await axios.get(`${API_BASE_URL}/articles/by-journalist/${journalistId}`);
  return response.data;
}
...
export async function getArticlesByCategory(categoryId) {
  const response = await axios.get(`${API_BASE_URL}/articles/by-category/${categoryId}`);
  return response.data;
}
